...
def ECEF_to_LatLon(p):
  ...
  return torch.stack([lat, lon], dim=0)
...
def LatLon_to_km(p, q):
  """Determine distance in km along the surface between to points"""
  a = p[0]-q[0]
  b = p[1]-q[1]
  d = np.arccos(np.sin(q[0])*np.sin(p[0])+np.cos(q[0])*np.cos(p[0])*np.cos(q[1]-p[1]))
...
def load_farm_data(device='cpu', dtype=torch.float32, save_new=True):
  ...
  if pathlib.Path(PRECOMPUTED_FARMS_DIR).exists():
    try:
      ...
      farm_data = tensors["farm_data"].to(device=device, dtype=dtype)
      if farm_data.dtype == dtype:
        return farm_data
    except Exception as e:
      print("Couldn't load farms. Reason:", e)
      pass
  ...
  if save_new:
    save_file({"farm_data": farm_data.cpu()}, PRECOMPUTED_FARMS_DIR) # save pre-computed array

  return farm_data
  
  
def get_farm_area_stats(farm_data=torch.zeros((3, 3)), device='cpu', dtype=torch.float32):
  old_farms = []
