...
from util_torch import load_farm_data, LatLon_to_ECEF, ECEF_to_Corotating, Tilted_to_ECI, ECI_to_Corotating, get_farm_area_stats
...
def orbital_efficiency(
  ...
  optimal_sellable_power = torch.zeros((points_per_day, orbit_phases, orbit_flavours, points_per_year, len(farm_types)), dtype=dtype, device=device)
  optimal_farm_distribution = torch.zeros((points_per_day, orbit_phases, orbit_flavours, points_per_year, len(farm_types)), dtype=dtype, device=device)
  ...
  for j, (tau_chunk, t_chunk) in enumerate(tqdm(zip(tau_chunks, t_chunks), total=len(tau_chunks))): 
    ...
    for k, type in enumerate(farm_types):
      ...
      max_transmission, optimal_farms = torch.max(sun_farm_transmission[:, :, :, farm_mask, :], dim=3)
  ...
  return optimal_sellable_power.cpu(), optimal_farm_distribution.cpu()
